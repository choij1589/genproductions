...
nproc=${3}
echo "%MSG-MG5 number of proc = $nproc"

maxevt=${4}
echo "%MSG-MG5 maxevt per proc = $maxevt"
...
max_events_per_iteration=5000000
...
while [ $produced_lhe -lt $nevt ]; do
  ...
  ./run.sh $submitting_event $run_random_seed -p $nproc -m $maxevt
